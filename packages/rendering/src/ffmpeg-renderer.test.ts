...
describe("ffmpeg renderer args", () => {
  it("builds video args that cut from source media, render 9:16, and burn subtitles", () => {
    expect(
      buildVideoArgs({
        candidate,
        sourceMedia,
        subtitlePath: "/tmp/clip_1.srt",
        outputVideoPath: "/tmp/clip_1.mp4",
  ...
  it("builds thumbnail args from the rendered clip midpoint", () => {
    expect(
      buildThumbnailArgs({
        candidate,
        outputVideoPath: "/tmp/clip_1.mp4",
  ...
  it("renders and stores downloadable video and thumbnail assets", async () => {
    ...
    expect(result).toEqual({
      ...
      value: {
        ...
        format: "mp4",
        aspectRatio: "9:16",
        videoUrl: "https://cdn.example.test/renders/sermon_1/clip_1.mp4",
